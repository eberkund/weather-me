...
pub mod weather_service_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with WeatherServiceServer.
    #[async_trait]
    ...
    ///
    #[derive(Debug)]
    pub struct WeatherServiceServer<T: WeatherService> {
        inner: Arc<T>,
    ...
    impl<T: WeatherService> WeatherServiceServer<T> {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for WeatherServiceServer<T>
    where
        T: WeatherService,
        B: Body + Send + 'static,
    ...
    impl<T: WeatherService> Clone for WeatherServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: WeatherService> tonic::server::NamedService for WeatherServiceServer<T> {
        const NAME: &'static str = "weather.v1.WeatherService";
    }
}
